<template>
    <div>
        ...
        <el-form class="main">
            ...
            <!--单选题-->
            <el-form-item v-for="(t,i) in optQuesList" v-bind:key="i" size="medium">
                <P class="quesTitle" :class="{ 'wrongAns': checkResult.optQuesRes[i] === false }">
                    <span class="quesNum">{{ t.number }}</span>{{ t.title }} <i
                    v-if="checkResult.optQuesRes[i] === false" class="el-icon-close"></i>
                </P>
                <el-image class="quesPic" v-if="t.has_pic" :src="t.pic_path" :fit="'scale-down'"/>
            ...
            <!--多选题-->
            <el-form-item v-for="(t,i) in multiOptQuestList" v-bind:key="i" size="medium">
                <P class="quesTitle" :class="{ 'wrongAns': checkResult.multiOptQuesRes[i] === false }">
                    <span class="quesNum">{{ t.number }}</span>（多选）{{ t.title }} <i
                    v-if="checkResult.multiOptQuesRes[i] === false" class="el-icon-close"></i>
                </P>
                <el-image class="quesPic" v-if="t.has_pic" :src="t.pic_path" :fit="'scale-down'"/>
            ...
            <!--判断题-->
            <el-form-item v-for="(t,i) in TFQuestionList" v-bind:key="i" size="medium">
                <P class="quesTitle" :class="{ 'wrongAns': checkResult.TFQuesRes[i] === false }">
                    <span class="quesNum">{{ t.number }}</span>{{ t.title }} <i
                    v-if="checkResult.TFQuesRes[i] === false" class="el-icon-close"></i>
                </P>
                <el-image class="quesPic" v-if="t.has_pic" :src="t.pic_path" :fit="'scale-down'"/>
            ...
            <!--填空题-->
            <el-form-item v-for="(t,i) in blankQuesList" v-bind:key="i">
                <P class="quesTitle" :class="{ 'wrongAns': checkResult.blanQuesRes[i] === false }">
                    <span class="quesNum">{{ t.number }}</span>{{ t.title }} <i
                    v-if="checkResult.blanQuesRes[i] === false" class="el-icon-close"></i>
                </P>
                <el-image class="quesPic" v-if="t.has_pic" :src="t.pic_path" :fit="'scale-down'"/>
            ...
            <!--编程题-->
            <P class="headline" v-if="programQuestList.length > 0">编程题<span
                style="font-size: 0.8em; font-weight: normal">（将代码文件上传即可）</span></P>
            <el-form-item v-for="(t,i) in programQuestList" v-bind:key="i">
                <P class="quesTitle"><span class="quesNum">{{ t.number }}</span>{{ t.title }}</P>
                <el-image class="quesPic" v-if="t.has_pic" :src="t.pic_path" :fit="'scale-down'"/>
                <el-upload class="upload-demo" drag :limit="1" :file-list="codefileList[i]" show-file-list
                           :auto-upload="false"
                           :on-error="uploadError" :on-success="uploadSuccess" action="#"
                           :on-change="(file, fileList)=>{return handleChange(file, fileList, i)}"
                           :on-remove="(file, fileList)=>{return fileRemove(file, fileList, i)}">
...
export default {
    ...
    mounted() {
            ...
            if (res.status === 200) {
                ...
                for (let e of res.data.data.questions) {
                    ...
                    switch (e.QuestionType) {
                        ...
                        case 0:
                        case 4: {
                            this.optQuestion.title = array[0].trim();
                            if (e.QuestionType === 4) {
                                this.optQuestion.options = array.slice(1, -1);
                                this.optQuestion.has_pic = true;
                                this.optQuestion.pic_path = array.slice(-1);
                            } else {
                                this.optQuestion.options = array.slice(1);
                                this.optQuestion.has_pic = false;
                            }
                        ...
                        case 2:
                        case 6: {
                            this.blankQuestion.title = array[0].trim();
                            if (e.QuestionType === 6) {
                                this.blankQuestion.options = array.slice(1, -1);
                                this.blankQuestion.has_pic = true;
                                this.blankQuestion.pic_path = array.slice(-1);
                            } else {
                                this.blankQuestion.options = array.slice(1);
                                this.blankQuestion.has_pic = false;
                            }
        ...
        // 获取编程题
        axios.request({
            url: `/chap${this.ChapterID}_test${this.TestID}.json`,
            method: "get"
        }).then(res => {
            res.data.forEach((v, i) => {
                this.blankQuestion.title = v.title.trim();
                this.blankQuestion.number = i + 1;
                if (v.has_pic === true)
                    this.blankQuestion.pic_path = v.pic_path;
